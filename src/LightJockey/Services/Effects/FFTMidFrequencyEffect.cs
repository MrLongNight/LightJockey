...
/// <summary>
/// FFT Mid Frequency effect using HTTPS API - mid-range reactive lighting
/// </summary>
public class FFTMidFrequencyEffect : IEffectPlugin
{
    private readonly ILogger<FFTMidFrequencyEffect> _logger;
    ...
    /// </summary>
    public string Name => "FFTMidFrequencyEffect";

    /// <summary>
    /// Gets a description of what this effect does
    /// </summary>
    public string Description => "HTTPS - Mid-range reactive lighting using mid frequency FFT data";
    ...
    /// <summary>
    /// Initializes a new instance of the FFTMidFrequencyEffect class
    /// </summary>
    /// <param name="logger">Logger instance</param>
    /// <param name="hueService">Hue service for light control</param>
    public FFTMidFrequencyEffect(ILogger<FFTMidFrequencyEffect> logger, IHueService hueService)
    {
    ...
    public async Task<bool> InitializeAsync(EffectConfig config)
    {
        ...
        try
        {
            ...
            if (_lights.Count == 0)
            {
                _logger.LogWarning("No lights available for FFTMidFrequencyEffect");
            }

            State = EffectState.Initialized;
            _logger.LogInformation("FFTMidFrequencyEffect initialized with {LightCount} lights", _lights.Count);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to initialize FFTMidFrequencyEffect");
            State = EffectState.Error;
            return false;
        }
    }
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("FFTMidFrequencyEffect started");
    ...
    public async Task StopAsync()
    {
        ...
        _logger.LogInformation("FFTMidFrequencyEffect stopped");
    }
    ...
    public void UpdateConfig(EffectConfig config)
    {
        ...
        _logger.LogDebug("FFTMidFrequencyEffect configuration updated");
    }
    ...
    public void OnSpectralData(SpectralDataEventArgs spectralData)
    {
        ...
        lock (_lock)
        {
            // React to mid frequency energy
            var sensitivity = _config.AudioSensitivity;
            _currentBrightness = Math.Clamp(spectralData.MidFrequencyEnergy * sensitivity * 2, 0.0, 1.0);
        }
    }
    ...
    private async Task RunEffectLoopAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Error in FFTMidFrequencyEffect loop");
    ...
    private async Task UpdateLightsAsync(CancellationToken cancellationToken)
    {
        ...
        // Green/yellow color for mid frequencies
        var hue = 120.0; // Green hue
