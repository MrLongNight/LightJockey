...
/// <summary>
/// FFT Low Frequency effect using HTTPS API - bass-reactive lighting
/// </summary>
public class FFTLowFrequencyEffect : IEffectPlugin
{
    private readonly ILogger<FFTLowFrequencyEffect> _logger;
    ...
    /// </summary>
    public string Name => "FFTLowFrequencyEffect";

    /// <summary>
    /// Gets a description of what this effect does
    /// </summary>
    public string Description => "HTTPS - Bass-reactive lighting using low frequency FFT data";
    ...
    /// <summary>
    /// Initializes a new instance of the FFTLowFrequencyEffect class
    /// </summary>
    /// <param name="logger">Logger instance</param>
    /// <param name="hueService">Hue service for light control</param>
    public FFTLowFrequencyEffect(ILogger<FFTLowFrequencyEffect> logger, IHueService hueService)
    {
    ...
    public async Task<bool> InitializeAsync(EffectConfig config)
    {
        ...
        try
        {
            ...
            if (_lights.Count == 0)
            {
                _logger.LogWarning("No lights available for FFTLowFrequencyEffect");
            }

            State = EffectState.Initialized;
            _logger.LogInformation("FFTLowFrequencyEffect initialized with {LightCount} lights", _lights.Count);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to initialize FFTLowFrequencyEffect");
            State = EffectState.Error;
            return false;
        }
    }
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("FFTLowFrequencyEffect started");
    ...
    public async Task StopAsync()
    {
        ...
        _logger.LogInformation("FFTLowFrequencyEffect stopped");
    }
    ...
    public void UpdateConfig(EffectConfig config)
    {
        ...
        _logger.LogDebug("FFTLowFrequencyEffect configuration updated");
    }
    ...
    public void OnSpectralData(SpectralDataEventArgs spectralData)
    {
        ...
        lock (_lock)
        {
            // React to low frequency energy (bass)
            var sensitivity = _config.AudioSensitivity;
            _currentBrightness = Math.Clamp(spectralData.LowFrequencyEnergy * sensitivity * 2, 0.0, 1.0);
        }
    }
    ...
    private async Task RunEffectLoopAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Error in FFTLowFrequencyEffect loop");
    ...
    private async Task UpdateLightsAsync(CancellationToken cancellationToken)
    {
        ...
        // Red/orange color for bass frequencies (low frequencies are typically red)
        var hue = 0.0; // Red hue
